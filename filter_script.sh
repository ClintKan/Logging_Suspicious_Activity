...
path="/home/ubuntu/analyz_logfiles_hw"

rm -f $path/suspicious_activity.log # to remove the file if it exists
...
for hit_word in "${hitword_group[@]}" #Using a for loop - where for every defined key hitword in the group of hitwords
  do
     cat /var/log/auth_log.log | grep -i $hit_word >> $path/suspicious_activity.log # print and add the whole entry-lines that have each of the hitword
  done
